...
import base64
import io
from cartoon_engine import enhanced_pipeline, emotion_detector, advanced_viz, multi_style
...
def cartoonize():
    try:
        ...
        # Style-specific path
        if style != 'default':
            result_img   = multi_style.apply_style(image, style)
            emotion_res  = emotion_detector.detect_emotion(image)
            identity_score = 0.0
            face_detected  = emotion_res.get('emotion', 'neutral') != 'neutral'
        else:
            ...
            emotion_res    = emotion_detector.detect_emotion(image)
            identity_score = float(metadata.get('identity_similarity', 0))
